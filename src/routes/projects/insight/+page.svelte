<article>
	<main>
		...
		<p>
			...
			of PDFs. Users can use the <a href="https://github.com/resolve-works/insight-ui">Web GUI</a>
			or <a href="https://github.com/resolve-works/insight-client">CLI client / Python library</a> to
			add document sets for their own use, or for the whole organization.
		</p>

		<p>
			It was created out of a need to research large FOIA requests without having to read them all.
			...
			to dig deeper when those bits are found.
		</p>
		...
		<p>
			The goal of insight is to provide insight into those documents. It aims to achieve that
			through the use of several technologies working in harmony.
		</p>

		<ul>
			<li>
				Keyword search. All documents are indexed and searchable by keyword. The search will
				highlight snippets of the pages that matched the query, and allows the user to navigate to
				hits directly.
			</li>
			<li>
				Semantic search. All documents are embedded and searchable semantically, which means that
				the user can prompt insight to return pages that are semantically similar to the users query
			</li>
			<li>
				Large language models. The results of the semantic search are fed into a LLM that will try
				to answer the users query based on the search results. The LLM is capable of producing
				quotes of the search results, which then can be fed into the keyword search.
			</li>
		</ul>
		...
		<p>
			...
			technologies, with a focus on user-friendly design and performance.
		</p>
	</main>
</article>
